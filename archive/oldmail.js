...
module.exports.mailMe = function( odbCn, jdeJob, cbWhenDone ) {
  ...
  log.debug( 'Mail request received for: ' + jdeJob + ' Report: ' + report );

  // Fetch email configuration for Report
  emailconfig.fetchMailDefaults( odbCn, jdeJob, cbWhenDone, report, '', processReportMailConfig );


}


function processReportMailConfig( err, mailConfig, odbCn, jdeJob, cbWhenDone ) {
  ...
  if ( err ) {
    ...
    emailconfig.fetchMailDefaults( odbCn, jdeJob, cbWhenDone, report, version, processVersionMailConfig );
...
function processVersionMailConfig( err, mailConfig, odbCn, jdeJob, cbWhenDone ) {
  ...
  mergeReportAndVersionOptions( cbWhenDone );

}


function mergeReportAndVersionOptions( cbWhenDone ) {

  log.debug( 'Report options: ' + emailConfig );
  log.debug( 'Version options: ' + emailConfigOverrides );

  emailconfig.mergeMailOptions( emailConfig, emailConfigOverrides, cbWhenDone, handleEmailing );

}


function handleEmailing( err, emailOptions, cbWhenDone ) {
